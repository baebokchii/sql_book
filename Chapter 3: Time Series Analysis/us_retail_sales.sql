...
FROM (
        SELECT
            YEAR(sales_month) AS sales_year, SUM(
                CASE
                    WHEN kind_of_business = 'Women''s clothing stores' THEN sales
                    ELSE 0
                END
...
FROM (
        SELECT YEAR(aa.sales_month) AS sales_year, bb.index_sales, SUM(aa.sales) AS sales
        ...
        GROUP BY
            YEAR(aa.sales_month), bb.index_sales
    ) aaa
ORDER BY sales_year;
...
FROM (
        ...
        GROUP BY
            YEAR(sales_month), kind_of_business
    ) a
ORDER BY sales_year, kind_of_business;
...
FROM
    ...
    AND b.sales_month BETWEEN DATE_SUB(
        a.sales_month,
        INTERVAL 11 MONTH
    ) AND a.sales_month
...
ORDER BY a.date, b.sales_month;

SELECT a.date, AVG(b.sales) AS moving_avg, COUNT(b.sales) AS records
FROM date_dim a
    JOIN (
        SELECT sales_month, sales
        FROM retail_sales
...
FROM (
    ...
    JOIN us_retail_sales b ON b.sales_month BETWEEN DATE_SUB(
        a.sales_month, INTERVAL 11 MONTH
    ) AND a.sales_month
    AND b.kind_of_business = 'Women''s clothing stores'
GROUP BY
    a.sales_month
...
SELECT sales_month, MONTH(sales_month) AS month_number
FROM us_retail_sales
WHERE
    kind_of_business = 'Book stores'
ORDER BY sales_month;
